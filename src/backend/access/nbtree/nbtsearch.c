...
#include "access/nbtree.h"       /* P_ISLEAF, BTPageOpaque, etc. */
			...
			Buffer buf)
{
    ...
    /* -------- Project: linear scan on tiny leaf pages -------- */
    if (btree_binsrch_linear && P_ISLEAF(opaque))
    {
        int nitems = (high >= low) ? (high - low + 1) : 0;

        /* Only do this for very small leaves: 2..threshold items */
        if (nitems >= 2 && nitems <= btree_binsrch_linear_threshold)
        {
            OffsetNumber off = low;

            if (!key->nextkey)            /* find first >= key */
            {
                while (off <= high &&
                       _bt_compare(rel, key, page, off) >= 1)
                    off = OffsetNumberNext(off);
            }
            else                           /* find first > key */
            {
                while (off <= high &&
                       _bt_compare(rel, key, page, off) >= 0)
                    off = OffsetNumberNext(off);
            }

            /* Match _bt_binsrch()’s leaf semantics for backward/forward */
            return key->backward ? OffsetNumberPrev(off) : off;
        }
    }
    /* ---------------------------------------------------------- */
			 ...
			 bool firstPage)
{
	...
	if (ScanDirectionIsForward(dir))
	{
        ...
        /* --- Project 1: leaf-page lookahead prefetch (forward) --- */
        if (btree_leaf_prefetch && P_ISLEAF(opaque) &&
            so->currPos.moreRight && !P_RIGHTMOST(opaque))
        {
	...
	else
	{
		...
		so->currPos.itemIndex = MaxTIDsPerBTreePage - 1;
		        /* --- Project 1: leaf-page lookahead prefetch (backward) --- */
        if (btree_leaf_prefetch && P_ISLEAF(opaque) &&
            so->currPos.moreLeft && !P_LEFTMOST(opaque))
        {
